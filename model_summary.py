...
def get_model_complexity_info(model, input_res, opt, print_per_layer_stat=True, as_strings=True,
                              input_constructor=None):
    ...
    if input_constructor:
        ...
        batch = torch.FloatTensor(1, *input_res).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        vsamples = {}
        ...
        for sample_type, sample_args in s_data_opt.items():
            vsamples[sample_type] = batch
        _ = flops_model(vsamples)
